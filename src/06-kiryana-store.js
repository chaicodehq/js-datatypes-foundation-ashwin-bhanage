...
export function getItemNames(items) {
    ...
    // .map() creates a new array by extracting just the names
...
export function sortByPrice(items, ascending) {
    ...
    // [...items] creates a shallow copy so we don't mutate the original list
...
export function formatBill(items) {
    ...
    // Step 1: Map each object to a string
    // Step 2: Join those strings with a newline character
    return items
        .map(
            (item) =>
